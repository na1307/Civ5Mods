...
//	--------------------------------------------------------------------------------
bool CvPlayer::canRaze(CvCity *pCity, bool bIgnoreCapitals, bool bIgnoreOwnership) const
{
    // No City Razing
    ...
    if (!bIgnoreOwnership && pCity->getOwner() != GetID())
    {
        return false;
    }

    // Can't raze a city that originally belonged to us
    if (!bIgnoreOwnership && pCity->getOriginalOwner() == GetID())
    {
        return false;
    }
...
void CvPlayer::receiveGoody(CvPlot *pPlot, GoodyTypes eGoody, CvUnit *pUnit)
{
    ...
    {
        iFaith = GetReligions()->GetCostNextProphet(false /*bIncludeBeliefDiscounts*/, true /*bAdjustForSpeedDifficulty*/, true) * iProphetPercent / 100;
        int iDivisor = /*10*/ GC.getGOLD_PURCHASE_VISIBLE_DIVISOR();
        iFaith /= iDivisor;
        iFaith *= iDivisor;
        ChangeFaith(iFaith);
    ...
    if (GetGreatPeopleSpawnCounter() > 0)
    {
        ...
        if (GetGreatPeopleSpawnCounter() == 0)
        {
            ...
            if (eBestMinor != NO_PLAYER)
                DoSpawnGreatPerson(eBestMinor, true);
                        ...
                        if (iNumFreeUnits > 0)
                        {
                            ...
                            for (int iUnitLoop = 0; iUnitLoop < iNumFreeUnits; iUnitLoop++)
                            {
                                ...
                                if (pNewUnit)
                                {
                                    ...
                                    {
                                        incrementGreatGeneralsCreated(true);
                                        pNewUnit->jumpToNearestValidPlot();
                                    }
                                    else if (pNewUnit->IsGreatAdmiral())
                                    {
                                        incrementGreatAdmiralsCreated(true);
                                    ...
                                    {
                                        incrementGreatWritersCreated(true);

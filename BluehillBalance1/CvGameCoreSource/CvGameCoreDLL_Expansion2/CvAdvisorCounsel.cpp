    ...
    for (int iTechLoop = 0; iTechLoop < pPlayerTechs->GetTechs()->GetNumTechs(); iTechLoop++)
    {
        ...
        if (pPlayerTechs->CanResearch(eTech) && pPlayerTechs->GetCurrentResearch() != eTech)
        {
            ...
            for (int iI = 0; iI < iNumBuildingInfos; iI++)
            {
                ...
                if (pBuilding && pBuilding->GetPrereqAndTech() == eTech && (BuildingTypes)(GET_PLAYER(ePlayer).getCivilizationInfo().getCivilizationBuildings(pBuilding->GetBuildingClassType())) == eBuilding && pBuilding->GetFlavorValue(eFlavorScience) > 0 && !GC.getGame().isBuildingClassMaxedOut((BuildingClassTypes)pBuilding->GetBuildingClassType()))
                {
            ...
            for (int i = 0; i < GC.getNumBuildingInfos(); i++)
            {
                ...
                if (!pBuildingEntry || pBuildingEntry->GetPrereqAndTech() != eTech || (BuildingTypes)(GET_PLAYER(ePlayer).getCivilizationInfo().getCivilizationBuildings(pBuildingEntry->GetBuildingClassType())) != eBuildingType)
                {
                    continue;
                }

...
void CvBuilderTaskingAI::AddChopDirectives(CvUnit* pUnit, CvPlot* pPlot, int iMoveTurnsAway)
{
	...
	for(int iBuildIndex = 0; iBuildIndex < GC.getNumBuildInfos(); iBuildIndex++)
	{
		...
		CvBuildInfo* pkBuild = GC.getBuildInfo(eBuild);
		if(NULL != pkBuild && pkBuild->getImprovement() == NO_IMPROVEMENT && pkBuild->isFeatureRemove(eFeature) && (pkBuild->getFeatureFood(eFeature) > 0 || pkBuild->getFeatureProduction(eFeature) > 0) && pUnit->canBuild(pPlot, eBuild))
		{
			eChopBuild = eBuild;
			break;
		}
	...
	pCity = NULL;
	int iFood = pPlot->getFeatureFood(eChopBuild, pUnit->getOwner(), &pCity);
	...
	iWeight += iBuildTimeWeight;
	iWeight *= (iFood + iProduction); // times the amount that the plot produces from the chopping

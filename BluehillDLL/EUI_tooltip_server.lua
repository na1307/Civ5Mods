...
local g_isShowCityStateLeaderIcon, g_isShowUnitIcon, g_isShowToolTipIcon, g_isBasicHelp, g_isScienceEnabled, g_isReligionEnabled, g_isHappinessEnabled, g_isPoliciesEnabled, g_isHealthEnabled, g_isEspionageDisabled, g_isAlwaysWar, g_isOneCityChallenge
...
do
	...
	local function UpdateOptions()
		...
		g_isShowCityStateLeaderIcon = UserInterfaceSettings.CityStateLeaders ~= 0
		g_isShowUnitIcon = UserInterfaceSettings.ShowUnitMouseOverIcon ~= 0
		g_UnitTooltipControls.PortraitFrame:SetHide( true )
		g_UnitTooltipControls.UnitPortrait:UnloadTexture()
		g_isShowToolTipIcon = UserInterfaceSettings.ShowToolTipIcon ~= 0
		g_ItemTooltipControls.PortraitFrame:SetHide( true )
		g_ItemTooltipControls.Portrait:UnloadTexture()
	end
...
--	if resourceID and GetResourceUsageType(resourceID) == RESOURCEUSAGE_STRATEGIC then
		...
		----------------------------
		insert( tips, "" )
		insert( tips, (numResourceUsed~=0 and (Colorize(-numResourceUsed) .. " ") or"") .. L"TXT_KEY_PEDIA_REQ_RESRC_LABEL" ) -- TXT_KEY_CITYVIEW_RESOURCE_DEMANDED
		tipIndex = #tips
		...
		if resourceHappiness > 0 and activePlayer:GetNumResourceAvailable( resourceID, true ) < 2 then
			local excessHappiness = activePlayer:GetExcessHappiness()
			if not activePlayer:IsEmpireUnhappy() then
				insert( tips, format( "%+i[ICON_HAPPINESS_1] (%i[ICON_HAPPINESS_1])", resourceHappiness, excessHappiness ) )
			elseif activePlayer:IsEmpireVeryUnhappy() then
				insert( tips, format( "%+i[ICON_HAPPINESS_1] (%i[ICON_HAPPINESS_4])", resourceHappiness, -excessHappiness ) )
			else
				insert( tips, format( "%+i[ICON_HAPPINESS_1] (%i[ICON_HAPPINESS_3])", resourceHappiness, -excessHappiness ) )
			end
		end
		for city in activePlayer:Cities() do
		...
		for minorPlayerID = MAX_MAJOR_CIVS, MAX_CIV_PLAYERS_minus1 do
			...
			if minorPlayer:IsAlive() and ( IsCiv5notVanilla or minorPlayer:GetActiveQuestForPlayer()==MINOR_CIV_QUEST_CONNECT_RESOURCE ) and minorPlayer:GetQuestData1(activePlayerID, MINOR_CIV_QUEST_CONNECT_RESOURCE) == resourceID then
...
local function ShowCivilizationToolTip( toolTip, playerID )
	...
	g_CivilizationTooltipControls.Grid:DoAutoSize()
	local isMinorCiv, isMajorCiv
	local player = Players[ playerID ]
	if player then
		if player:IsMinorCiv() then
			local minorCivInfo = GameInfo.MinorCivilizations[ player:GetMinorCivType() ]
			if minorCivInfo and g_isShowCityStateLeaderIcon then
				local textureKey = "TXT_KEY_CSL_ICON_"..minorCivInfo.Type
				if Locale.HasTextKey( textureKey ) then
...
--					g_CivilizationTooltipControls.Leader:SetTextureOffsetVal( 0, 0 )
					isMinorCiv = g_CivilizationTooltipControls.Leader:SetTexture( L(textureKey) )
--[[
				else
	...
	Score = function( playerID )
		...
		if player then
			...
			for i = 1, 4 do
				...
				if f and Locale.HasTextKey( key ) then
					insert( tips, L( key, f(player) ) )
				end
			end
			return concat( tips, "[NEWLINE]" )
		end
	...
	if unit and activePlayer and activeTeam and action then
		...
		if isBuild then
			...
			if feature and unitPlot:IsBuildRemovesFeature(buildID) then
				...
				local featureFood = unitPlot:GetFeatureFood( buildID, activeTeamID )
				local featureProduction = unitPlot:GetFeatureProduction( buildID, activeTeamID )
				if featureFood > 0 and featureProduction > 0 then
					tip = tip .. L( "TXT_KEY_BUILD_FEATURE_FOOD_PRODUCTION", featureFood, featureProduction )
					local city = unitPlot:GetWorkingCity()
					if city then
						tip = tip .. " (".. city:GetName()..")"
					end
				elseif featureFood > 0 then
					tip = tip .. L( "TXT_KEY_BUILD_FEATURE_FOOD", featureFood )
					local city = unitPlot:GetWorkingCity()
					if city then
						tip = tip .. " (".. city:GetName()..")"
					end
				elseif featureProduction > 0 then
...
local CityToolTips = {
		...
		elseif activeTeam:IsHasMet( cityTeamID ) then
			...
			if cityOwner.GetWarmongerPreviewString and activeTeam:IsAtWar( cityTeamID ) then
				tip = tip .. "[NEWLINE]"----------------[NEWLINE]"
						.. cityOwner:GetWarmongerPreviewString( city:GetOwner(), city, GetActivePlayer() ) --CPP
				if city:GetOriginalOwner() ~= city:GetOwner() then
					tip = tip .. "[NEWLINE]"----------------[NEWLINE]"
							.. cityOwner:GetLiberationPreviewString( city:GetOriginalOwner(), city, GetActivePlayer() ) --CPP
				end
	...
	BuildGrowth = function( city )
		return CityResist( city ) or ShowTextToolTip( L( "TXT_KEY_CITY_CURRENTLY_PRODUCING_TT", city:GetName(), city:GetProductionNameKey(), city:GetProductionTurnsLeft() ), GetProductionTooltip( city ) )
	end,
	...
	BorderGrowth = function( city )
		return CityResist( city ) or ShowTextToolTip( -- city:GetName(),
...
LuaEvents.CityToolTips.Add( function( control, city )
	...
	return toolTip and ShowTextToolTip( toolTip ) --city:GetName(), 
end)
...
local TopPanelTooltips = {
	...
	HappinessString = function()
		...
		if g_isHappinessEnabled then
			...
			local leagueHappiness = activePlayer.GetHappinessFromLeagues and activePlayer:GetHappinessFromLeagues() or 0 -- IsCiv5BNW_BE
			local eventHappiness = activePlayer.GetEventHappiness and activePlayer:GetEventHappiness() or 0 --CPP
			...
			-- Happiness from events (CPP)
			insertLocalizedBulletIfNonZero( tips, "          ", "TXT_KEY_TP_HAPPINESS_EVENT", eventHappiness )
...
--			insert( tips, missingResources )
	...
	InternationalTradeRoutes = function()
		...
		numAvailableTradeUnits = activePlayer:GetNumAvailableTradeUnits(DomainTypes.DOMAIN_SEA)
		if numAvailableTradeUnits > 0 then

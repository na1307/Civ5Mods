...
bool CvGameReligions::CheckSpawnGreatProphet(CvPlayer& kPlayer)
{
	...
	const int iFaith = kPlayer.GetFaith();
	int iCost = kPlayer.GetReligions()->GetCostNextProphet(true /*bIncludeBeliefDiscounts*/, true /*bAdjustForSpeedDifficulty*/, GC.getGame().isOption(GAMEOPTION_TRULY_FREE_GP));
...
void CvPlayerReligions::Read(FDataStream& kStream)
{
	...
	uint uiVersion;
	kStream >> uiVersion;
	kStream >> m_iNumFreeProphetsSpawned;

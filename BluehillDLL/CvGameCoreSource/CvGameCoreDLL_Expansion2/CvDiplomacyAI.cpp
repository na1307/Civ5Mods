...
CvPlayer* CvDiplomacyAI::GetPlayer() const
{
	return m_pPlayer;
}
...
int CvDiplomacyAI::GetMajorCivOpinionWeight(PlayerTypes ePlayer)
{
	...
	iOpinionWeight += GetEmbassyThemScore(ePlayer);
...
int CvDiplomacyAI::GetEmbassyThemScore(PlayerTypes ePlayer)
{
	int iOpinionWeight = 0;
	if (GET_TEAM(GET_PLAYER(ePlayer).getTeam()).HasEmbassyAtTeam(GetTeam()))
		iOpinionWeight += (/*0*/ GC.getOPINION_WEIGHT_EMBASSY_THEM());
	return iOpinionWeight;
}
...
int CvDiplomacyAI::GetDiplomatScore(PlayerTypes ePlayer)
{
	...
	if (GetPlayer()->GetEspionage()->IsOtherDiplomatVisitingMe(ePlayer))
		iOpinionWeight += (/*0*/ GC.getOPINION_WEIGHT_DIPLOMAT());
	return iOpinionWeight;
}
...
int CvDiplomacyAI::GetOpenBordersScore(PlayerTypes ePlayer)
{
	...
	if (GET_TEAM(GET_PLAYER(ePlayer).getTeam()).IsAllowsOpenBordersToTeam(GetTeam()))
		iOpinionWeight = /*0*/ GC.getOPINION_WEIGHT_OPEN_BORDERS_US();

...
bool CvPlot::canBuild(BuildTypes eBuild, PlayerTypes ePlayer, bool bTestVisible, bool bTestPlotOwner) const
{
	...
	{
		    if((!GC.getGame().isOption(GAMEOPTION_ONLY_REPAIR_TEAM_TILES) || isOwned() && getTeam() == eTeam) && (IsImprovementPillaged() || IsRoutePillaged()))
		    {
			    bValid = true;
		    }
		    else
...
int CvPlot::getFeatureFood(BuildTypes eBuild, PlayerTypes ePlayer, CvCity** ppCity) const
{
	...
	*ppCity = getWorkingCity();
	...
	// Base value
	if (GET_PLAYER(ePlayer).GetAllFeatureFood() > 0)
	{
		iFood = GET_PLAYER(ePlayer).GetAllFeatureFood();
	}
	else
	{
	...
	iFood *= std::max(0, (GET_PLAYER((*ppCity)->getOwner()).getFeatureFoodModifier() + 100));
	iFood /= 100;

	iFood *= GC.getGame().getGameSpeedInfo().getFeatureProductionPercent();
	iFood /= 100;

	if (GC.getGame().isOption(GAMEOPTION_PROGRESSIVE_FEATURE_CLEAR))
	{
		...
		if(getBuildProgress(eBuild) >= getBuildTime(eBuild, ePlayer))
		{
			...
			if(getFeatureType() != NO_FEATURE)
			{
				if(pkBuildInfo->isFeatureRemove(getFeatureType()))
				{
					...
					iFood = getFeatureFood(eBuild, ePlayer, &pCity);

...
int CvLuaPlayer::lGetMinimumFaithNextGreatProphet(lua_State* L)
{
	...
	int iFaith = pkPlayer->GetReligions()->GetCostNextProphet(true /*bIncludeBeliefDiscounts*/, true /*bAdjustForSpeedDifficulty*/, GC.getGame().isOption(GAMEOPTION_TRULY_FREE_GP));
...
//------------------------------------------------------------------------------
//int getFeatureFoodModifier();
int CvLuaPlayer::lGetFeatureFoodModifier(lua_State* L)
{
	return BasicLuaMethod(L, &CvPlayerAI::getFeatureFoodModifier);
}
//------------------------------------------------------------------------------
...
int CvLuaPlayer::lGetOpinionTable(lua_State* L)
{
	...
	iValue = pDiploAI->GetEmbassyThemScore(eWithPlayer);
	if (iValue != 0)
	{
		Opinion kOpinion;
		kOpinion.m_iValue = iValue;
	...
	iValue = pDiploAI->GetDiplomatScore(eWithPlayer);
	if (iValue != 0)
	{
		Opinion kOpinion;
		kOpinion.m_iValue = iValue;
	...
	for (uint ui = 0; ui < aOpinions.size(); ui++)
	{
		...
		// Diplomacy Values
		if (GC.getGame().isOption(GAMEOPTION_DIPLOMACY_VALUES))
		{
			CvString sValue;
			...
			{
				sValue.Format(" (+%d)", -(aOpinions[ui].m_iValue));
			} 
			else
			{
				sValue.Format(" (%d)", -(aOpinions[ui].m_iValue));

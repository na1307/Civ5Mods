...
int CvMinorCivAI::GetFriendshipChangePerTurnTimes100(PlayerTypes ePlayer)
{
	...
	{
		// Friendly Minors have Friendship decay slower
		if (GC.getGame().isOption(GAMEOPTION_FRIENDSHIP_DECAY_SLOWER) && GetPersonality() == MINOR_CIV_PERSONALITY_FRIENDLY)
			iChangeThisTurn += /*-50*/ GC.getMINOR_FRIENDSHIP_DROP_PER_TURN_FRIENDLY();
...
int CvMinorCivAI::CalculateBullyMetric(PlayerTypes eBullyPlayer, bool bForUnit, CvString* sTooltipSink)
{
	...
	// -300 ~ -0
	// **************************
	if (GC.getGame().isOption(GAMEOPTION_RESISTANCE_TO_BULLYING) && GetGiveGoldQuestCleared() > 0)
	{

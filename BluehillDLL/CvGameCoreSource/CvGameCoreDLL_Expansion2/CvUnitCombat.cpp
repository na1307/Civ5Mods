...
void CvUnitCombat::ResolveCityMeleeCombat(const CvCombatInfo &kCombatInfo, uint uiParentEventID)
{
    ...
    if (pkAttacker && pkDefender)
    {
        ...
        // Barbarian and Minor civs can't conquest cities
        else if ((pkAttacker->isBarbarian() || GET_PLAYER(pkAttacker->getOwner()).isMinorCiv()) && (pkDefender->getDamage() >= pkDefender->GetMaxHitPoints()) && !pkAttacker->isNoCapture())
        {
            if (pkAttacker->isBarbarian() && pkDefender->GetPlayer()->GetID() == GC.getGame().getActivePlayer() && pkDefender->isHuman() && !GC.getGame().isGameMultiPlayer())
            {
                gDLL->UnlockAchievement(ACHIEVEMENT_REALLY_SUCK);
            }

            if (GET_PLAYER(pkDefender->getOwner()).canRaze(pkDefender, false, true))
            {
                if (pkAttacker->getOwner() == iActivePlayerID)
                {
                ...
                GET_PLAYER(pkDefender->getOwner()).disband(pkDefender);
        ...
        // City conquest
        else if (pkDefender->getDamage() >= pkDefender->GetMaxHitPoints() && !pkAttacker->isNoCapture())
        {
            if (pkAttacker->getOwner() == iActivePlayerID)
            {
                strBuffer = GetLocalizedText("TXT_KEY_MISC_YOU_UNIT_CAPTURED_ENEMY_CITY", pkAttacker->getNameKey(), iDefenderDamageInflicted, pkDefender->getNameKey());

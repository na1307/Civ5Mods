	...
	if(bImprovement)
	{
		...
		if(pkImprovement)
		{
			if(pPlot->getTeam() != getTeam())
			{
				...
				if (GC.getGame().isOption(GAMEOPTION_NO_RANDOMIZE_PILLAGE_GOLD))
				{
					iPillageGold = (int)(pkImprovement->GetPillageGold() * 1.5);
				}
				else
				{
...
int CvUnit::getTradeInfluence(const CvPlot* pPlot) const
{
	...
	if (pPlot && canTrade(pPlot))
	{
		...
		if (eMinor != NO_PLAYER)
		{
			...
			// Progressive Great Merchant Influences
			if (GC.getGame().isOption(GAMEOPTION_PROGRESSIVE_GM_INFLUENCES))
			{
				double fDiscoveredTechs = GET_TEAM(getTeam()).GetTeamTechs()->GetNumTechsKnown() - 1;
				double fAllTechs = GC.GetGameTechs()->GetNumTechs() - 2; // Except Agriculture and Future Tech
				iInf += (int)((fDiscoveredTechs / fAllTechs) * 100);
			}

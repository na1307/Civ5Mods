				...
				if(GC.getGame().countKnownTechNumTeams(eIndex) == 1)
				{
					if(pkTechInfo->GetFirstFreeUnitClass() != NO_UNITCLASS)
					{
						...
						if(eFreeUnit != NULL)
						{
							...
							{
								pCapitalCity->GetCityCitizens()->DoSpawnGreatPerson(eFreeUnit, true, false, GC.getGame().isOption(GAMEOPTION_TRULY_FREE_GP));
							}
						}
					}
...
void CvTeam::processTech(TechTypes eTech, int iChange)
{
	...
	for(iI = 0; iI < MAX_PLAYERS; iI++)
	{
		...
		{
			kPlayer.changeFeatureFoodModifier(pTech->GetFeatureFoodModifier() * iChange);

		...
		if(kPlayer.GetCurrentEra() >= GC.getInfoTypeForString("ERA_INDUSTRIAL", true /*bHideAssert*/))
		{
			...
			if (eUnitClass != NO_UNITCLASS)
			{
				...
				if (eUnitClass == GC.getInfoTypeForString("UNITCLASS_PROPHET", true /*bHideAssert*/)) //here
				{
					...
					{
						iCost = kPlayer.GetReligions()->GetCostNextProphet(true /*bIncludeBeliefDiscounts*/, false /*bAdjustForSpeedDifficulty*/, GC.getGame().isOption(GAMEOPTION_TRULY_FREE_GP));
					}
				}
			...
			for(int iUnitLoop = 0; iUnitLoop < GC.getNumUnitInfos(); iUnitLoop++)
			{
				...
				if(pkUnitInfo)
				{
					for(iFreeUnitLoop = 0; iFreeUnitLoop < pBuildingInfo->GetNumFreeUnits(iUnitLoop); iFreeUnitLoop++)
					{
						...
						{
							GetCityCitizens()->DoSpawnGreatPerson(eFreeUnitType, true /*bIncrementCount*/, true, GC.getGame().isOption(GAMEOPTION_TRULY_FREE_GP));
						}
						else
						{
							...
							{
								owningPlayer.incrementGreatGeneralsCreated(GC.getGame().isOption(GAMEOPTION_TRULY_FREE_GP));
								if (!pFreeUnit->jumpToNearestValidPlot())
									pFreeUnit->kill(false);	// Could not find a valid spot!
							}
							else if(pFreeUnit->IsGreatAdmiral())
							{
								owningPlayer.incrementGreatAdmiralsCreated(GC.getGame().isOption(GAMEOPTION_TRULY_FREE_GP));
							...
							{
								owningPlayer.incrementGreatWritersCreated(GC.getGame().isOption(GAMEOPTION_TRULY_FREE_GP));
								if (!pFreeUnit->jumpToNearestValidPlot())
									pFreeUnit->kill(false);	// Could not find a valid spot!
							}							
							else if (pkUnitInfo->GetUnitClassType() == GC.getInfoTypeForString("UNITCLASS_ARTIST"))
							{
								owningPlayer.incrementGreatArtistsCreated(GC.getGame().isOption(GAMEOPTION_TRULY_FREE_GP));
								if (!pFreeUnit->jumpToNearestValidPlot())
									pFreeUnit->kill(false);	// Could not find a valid spot!
							}							
							else if (pkUnitInfo->GetUnitClassType() == GC.getInfoTypeForString("UNITCLASS_MUSICIAN"))
							{
								owningPlayer.incrementGreatMusiciansCreated(GC.getGame().isOption(GAMEOPTION_TRULY_FREE_GP));
								if (!pFreeUnit->jumpToNearestValidPlot())
									pFreeUnit->kill(false);	// Could not find a valid spot!
							}							
							else if (pFreeUnit->IsGreatPerson())
							{
								if (GC.getGame().isOption(GAMEOPTION_SEPARATE_GP_COUNTERS))
								{
									...
									{
										owningPlayer.incrementGreatMerchantsCreated(GC.getGame().isOption(GAMEOPTION_TRULY_FREE_GP));
									}
									else if (pkUnitInfo->GetUnitClassType() == GC.getInfoTypeForString("UNITCLASS_SCIENTIST"))
									{
										owningPlayer.incrementGreatScientistsCreated(GC.getGame().isOption(GAMEOPTION_TRULY_FREE_GP));
									}
									else
									{
										owningPlayer.incrementGreatEngineersCreated(GC.getGame().isOption(GAMEOPTION_TRULY_FREE_GP));
									}
								}
								else
								{
									owningPlayer.incrementGreatPeopleCreated(GC.getGame().isOption(GAMEOPTION_TRULY_FREE_GP));
								}
